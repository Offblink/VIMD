...
async def main():
    ...
    # ── 文件被外部程序 (agent) 改过: 不弹恢复框, 草稿按当前内容同步 ──
    def _seed_draft(path, content, ts=None):
        RECOVERY.parent.mkdir(parents=True, exist_ok=True)
        key = appmod._recovery_key(path)
        entry = {"path": str(path), "content": content}
        if ts is not None:
            entry["ts"] = ts
        RECOVERY.write_text(_json.dumps({"entries": {key: entry}},
                                        ensure_ascii=False),
                            encoding="utf-8")
        return key
    ...
    def _read_entry(key):
        return _json.loads(
            RECOVERY.read_text(encoding="utf-8"))["entries"][key]

    async def external_round():
        # ① 草稿比文件旧 = 关掉 VIMD 期间 agent 改过文件 -> 不弹框, 草稿跟到当前
        target = SCRATCH / "external.md"
        target.write_text("磁盘上的新内容 (agent 改的)", encoding="utf-8")
        key = _seed_draft(target, "关掉 VIMD 之前的旧草稿",
                          target.stat().st_mtime - 10)
        app5 = VIMDApp(path=str(target))
        async with app5.run_test(size=(100, 30)) as p5:
            await p5.pause(0.7)
            check("外部改过: 不弹恢复框",
                  not isinstance(app5.screen, RecoveryPrompt))
            ed5 = app5.query_one(Editor)
            check("外部改过: 编辑区是文件当前内容", "agent 改的" in ed5.text)
            check("外部改过: 底栏无恢复按钮",
                  not app5.query_one("#hint-recover").display)
            # 同步动作本身: 复现刚才那次检查 (不 await —— 免得 0.5s 后的
            # "干净态删草稿"定时器插进来), 直接看盘上的条目
            _seed_draft(target, "关掉 VIMD 之前的旧草稿",
                        target.stat().st_mtime - 10)
            app5._check_recovery()
            ent = _read_entry(key)
            check("外部改过: 草稿写成当前内容",
                  ent["content"] == ed5.text
                  and "旧草稿" not in ent["content"])
            check("外部改过: 草稿时间戳抬到文件之后",
                  ent["ts"] >= target.stat().st_mtime)
            check("外部改过: 不同步成待恢复态", app5._recovery_data is None)

        # 草稿已是当前内容 -> 重开一次不该再问
        app5b = VIMDApp(path=str(target))
        async with app5b.run_test(size=(100, 30)) as p5b:
            await p5b.pause(0.6)
            check("外部改过: 重开不再问",
                  not isinstance(app5b.screen, RecoveryPrompt))

        # ② 反向对照: 草稿比文件新 = 崩溃丢的未保存内容 -> 仍旧弹恢复框
        crash = SCRATCH / "crashish.md"
        crash.write_text("磁盘上的已存内容", encoding="utf-8")
        key2 = _seed_draft(crash, "崩溃前未保存的改动",
                           crash.stat().st_mtime + 10)
        app6 = VIMDApp(path=str(crash))
        async with app6.run_test(size=(100, 30)) as p6:
            await p6.pause(0.7)
            check("草稿比文件新: 仍弹恢复框",
                  isinstance(app6.screen, RecoveryPrompt))
            check("草稿比文件新: 草稿没被覆盖",
                  _read_entry(key2)["content"] == "崩溃前未保存的改动")
            await p6.press("escape")

    await external_round()
    try:
        RECOVERY.unlink()
    except OSError:
        pass
