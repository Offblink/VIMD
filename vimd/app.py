...
视图模式 (持久化到 %APPDATA%/VIMD/settings.json):
    Alt+1 编辑 · Alt+2 预览 · Alt+3 分屏 · Alt+H 帮助 · Alt+L 行号 · Ctrl+R 重载 · Alt+Q 退出
"""
...
class VIMDApp(App):
    ...
    BINDINGS = [
        ...
        # Ctrl+R 重载 (2026-09-27 用户改绑, 原 Alt+S): TextArea / Input / App 都没绑
        # 这个键, 本来就能抢到; 仍标 priority 是为了焦点在任何输入框里都先走应用层,
        # 与查找导航那两条一个口径
        Binding("ctrl+r", "reload", "重载", show=False, priority=True),
    ...
    # ── 重新加载 (Ctrl+R): 外部程序改过文件时手动同步 ────────
    def action_reload(self) -> None:
        """Ctrl+R: 丢掉缓冲区, 重新从磁盘读本文件。
